...
namespace Aurora.Profiles.Payday_2
{
    ...
    public partial class Control_PD2 : UserControl
    {
        ...
        private void preview_gamestate_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (IsLoaded)
            {
                (profile_manager.Config.Event._game_state as GameState_PD2).Game.State = (GSI.Nodes.GameStates)this.preview_gamestate.SelectedValue;
            }
        }
        ...
        private void preview_levelphase_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (IsLoaded)
            {
                (profile_manager.Config.Event._game_state as GameState_PD2).Level.Phase = (GSI.Nodes.LevelPhase)this.preview_levelphase.SelectedValue;
            }
        }
        ...
        private void preview_playerstate_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (IsLoaded)
            {
                (profile_manager.Config.Event._game_state as GameState_PD2).Players.LocalPlayer.State = (GSI.Nodes.PlayerState)this.preview_playerstate.SelectedValue;
            }
        }
